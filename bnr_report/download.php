...
//Eligible deposits
$query = "SELECT a.cli,a.nomrest,c.dev,sum(c.sdecv) tot,R.TIND rate, count(c.sdecv) num  from prod.bkcli a 
join prod.bksld c on c.cli=a.cli and c.dco=?
JOIN prod.bktau R ON R.DEV=C.DEV and R.dco=?
where (
    c.cha like ? or 
    c.cha like ? or 
    c.cha like ? OR 
    c.cha like ? or 
    c.cha like ?) and c.sde>?-- and c.cfe<>'O'
group by a.cli,a.nomrest,c.dev,R.TIND";
...
$rate = array();
$num8 = array();
if ($stmt->execute(array($dco,$dco,'201%','1220%','2030%','204%','205%',0)))
{
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $cli8[] = (string)$row['CLI'];
        $nomrest8[] = (string)$row['NOMREST'];
        $dev8[] = (int)$row['DEV'];
        $tot[] = (float)$row['TOT'];
        $rate[] = (float)$row['RATE'];
        $num8[] = (int)$row['NUM'];
...
$objPHPExcel->setActiveSheetIndex(0)
->setCellValue('C14',$num1);

$objPHPExcel->setActiveSheetIndex(0)
->setCellValue('D14',$tot1);
// $objPHPExcel->setActiveSheetIndex(7)
// ->fromArray(
//     array_chunk($dev4, 1),
//     NULL,        // Array values with this value will not be set
